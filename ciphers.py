    ...
    for line in usr_file.readlines():
        ...
        for j, i in enumerate(line.lower()):
            ...
            if i not in letters_en and i not in letters_ru:
                ...
                new_txt += letters_ru[(letters_ru.find(i) + step * letters_ru.find(args[2][(j - spase_couner) % len_key])) % len_ru]
            else:
                new_txt += letters_en[(letters_en.find(i) + step * letters_en.find(args[2][(j - spase_couner) % len_key])) % len_en]
        cipher.write(new_txt)
    usr_file.close()
    cipher.close()
    return "Result in " + args[3]
